...
function sendProposalMessage(leadId) {
...
function leadActionMenuHtml(lead) {
  return `
    ...
    ${lead.phone && !["Matriculado", "Perdido"].includes(lead.status) ? `<button class="row-action-menu-item" data-send-proposal-lead="${lead.id}" type="button"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M22 2 11 13"/><path d="M22 2 15 22l-4-9-9-4 20-7z"/></svg> Enviar proposta</button>` : ""}
...
document.addEventListener("click", (event) => {
  ...
  const sendProposalButton = event.target.closest("[data-send-proposal-lead]");
  if (sendProposalButton) sendProposalMessage(sendProposalButton.dataset.sendProposalLead);

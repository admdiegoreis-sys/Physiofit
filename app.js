            ...
            <tr>
              <td>
                <div class="lead-row-actions">
                  <button class="row-action-button lead-edit-button" data-edit-lead="${lead.id}" type="button" title="Editar lead" aria-label="Editar lead"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
...
let _proposalLeadId = null;

function openProposalOverlay(leadId) {
  const lead = state.leads.find((l) => l.id === leadId);
  if (!lead) return;
  _proposalLeadId = leadId;
  document.querySelector("#proposalLeadName").textContent = lead.name;
  document.querySelector("#proposalSendWhatsapp").hidden = !lead.phone;
  document.querySelector("#proposalSendOverlay").hidden = false;
}

function closeProposalOverlay() {
  document.querySelector("#proposalSendOverlay").hidden = true;
  _proposalLeadId = null;
}
...
function sendProposalByWhatsapp() {
  const lead = state.leads.find((l) => l.id === _proposalLeadId);
  ...
  const interest = lead.interest ? ` para ${lead.interest}` : "";
  const message = `Olá ${lead.name}! Segue nossa proposta${interest} aqui na Physiofit Studio. Fico à disposição para tirar qualquer dúvida!`;
  window.open(`https://wa.me/${waNumber}?text=${encodeURIComponent(message)}`, "_blank", "noopener");

  markProposalSent(lead.id);
  closeProposalOverlay();
  toast("Proposta enviada — status atualizado.");
}

function sendProposalManually() {
  if (!_proposalLeadId) return;
  markProposalSent(_proposalLeadId);
  closeProposalOverlay();
  toast("Proposta marcada como enviada.");
}

document.querySelector("#proposalSendWhatsapp")?.addEventListener("click", sendProposalByWhatsapp);
document.querySelector("#proposalSendManual")?.addEventListener("click", sendProposalManually);
document.querySelector("#proposalSendCancel")?.addEventListener("click", closeProposalOverlay);
document.querySelector("#proposalSendOverlay")?.addEventListener("click", (e) => {
  if (e.target.id === "proposalSendOverlay") closeProposalOverlay();
});

// Ordem alinhada às etapas do funil (Contato → Visita → Proposta → Cadastro/Matrícula → Perdido).
// "Editar lead" fica fora deste menu (botão próprio ao lado do "...").
function leadActionMenuHtml(lead) {
  return `
    ${lead.phone ? `<a class="row-action-menu-item" data-mark-contact-lead="${lead.id}" href="${whatsappLeadUrl(lead.phone)}" target="_blank" rel="noopener noreferrer"><svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413z"/></svg> WhatsApp</a>` : ""}
    ...
    ${lead.phone && !["Matriculado", "Perdido"].includes(lead.status) ? `<button class="row-action-menu-item" data-open-proposal-lead="${lead.id}" type="button"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M22 2 11 13"/><path d="M22 2 15 22l-4-9-9-4 20-7z"/></svg> Enviar proposta</button>` : ""}
...
document.addEventListener("click", (event) => {
  ...
  const openProposalButton = event.target.closest("[data-open-proposal-lead]");
  if (openProposalButton) openProposalOverlay(openProposalButton.dataset.openProposalLead);

  // Clicar em "WhatsApp" pra chamar o lead já conta como contato iniciado — não bloqueia
  // a navegação (o link abre normalmente), só atualiza o status se ainda estava em "Novo lead".
  const markContactLink = event.target.closest("[data-mark-contact-lead]");
  if (markContactLink) {
    const cLead = state.leads.find((l) => l.id === markContactLink.dataset.markContactLead);
    if (cLead && cLead.status === "Novo lead") {
      cLead.status = "Contato iniciado";
      saveState();
      renderCrm();
    }
  }
